            ...
            .then(data => {
                ...
                if (!Array.isArray(data) || data.length === 0) {
                    ...
                    const sortedData = data.slice().sort((a, b) => {
                        const aZero = (a.mtdTotalTarget === 0) ? 1 : 0;
                        const bZero = (b.mtdTotalTarget === 0) ? 1 : 0;
                        if (aZero !== bZero) return aZero - bZero;
                        return (b.complianceRate || 0) - (a.complianceRate || 0);
                    });

                    empRows.innerHTML = sortedData.map((item, idx) => {
                        const name = item.karyawanName || '-';
                        const nik = item.nik || '-';
                        const jabatan = item.jabatanName || '-';
                        
                        const mtd = Number(item.complianceRate || 0).toFixed(1);
                        
                        function formatZeroTgt(actual, target) {
                            if (target === 0) return '<span style="font-weight: 900; color: #000000;">' + actual + ' / ' + target + '</span>';
                            return actual + ' / ' + target;
                        }

                        const hazard = formatZeroTgt(item.hazard ? item.hazard.actual : 0, item.hazard ? item.hazard.target : 0);
                        const inspeksi = formatZeroTgt(item.inspeksi ? item.inspeksi.actual : 0, item.inspeksi ? item.inspeksi.target : 0);
                        const safetyTalk = formatZeroTgt(item.safetyTalk ? item.safetyTalk.actual : 0, item.safetyTalk ? item.safetyTalk.target : 0);
                        const observasi = formatZeroTgt(item.observasi ? item.observasi.actual : 0, item.observasi ? item.observasi.target : 0);
                        const coaching = formatZeroTgt(item.coaching ? item.coaching.actual : 0, item.coaching ? item.coaching.target : 0);
                        const p5m = formatZeroTgt(item.p5m ? item.p5m.actual : 0, item.p5m ? item.p5m.target : 0);
                        
                        const isZeroTarget = (item.mtdTotalTarget === 0);
                        const rowStyle = isZeroTarget ? 'background-color: #f0fdf4;' : '';
    ...
    window.openDeptModal = function(companyId, companyName, deptData) {
        ...
        if (!Array.isArray(deptData) || deptData.length === 0) {
            ...
            const sortedDeptData = deptData.slice().sort(function(a, b) {
                const mtdA = Number(a.mtdAchievementRate || a.MtdAchievementRate || 0);
                const mtdB = Number(b.mtdAchievementRate || b.MtdAchievementRate || 0);
                return mtdB - mtdA;
            });
            ...
            deptRows.innerHTML = sortedDeptData.map(function (item, idx) {
                ...
                const hRate = Number(item.mtdHazardRate || item.MtdHazardRate || 0).toFixed(1);
                const iRate = Number(item.mtdInspeksiRate || item.MtdInspeksiRate || 0).toFixed(1);
                const stRate = Number(item.mtdSafetyTalkRate || item.MtdSafetyTalkRate || 0).toFixed(1);
                const oRate = Number(item.mtdObservasiRate || item.MtdObservasiRate || 0).toFixed(1);
                const cRate = Number(item.mtdCoachingRate || item.MtdCoachingRate || 0).toFixed(1);
                const p5mRate = Number(item.mtdP5mRate || item.MtdP5mRate || 0).toFixed(1);
